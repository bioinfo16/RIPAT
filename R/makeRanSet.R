...
#' 
#' @param organism a single character. This function can run by two versions of organisms\cr
#'                 such as GRCh37, GRCh38 (Human). Default is 'GRCh37'.
#' @param randomSize an integer vector. A random set size. Default is 10000.
#' @param outPath an string vector. Output file is saved in this path.\cr Default value is R home directory.
...
#' 
#' @export
makeRanSet = function(organism = 'GRCh37', randomSize = if(doRandom){10000}else{NULL}, outPath = getwd(), outFileName = paste0('RIPAT', round(unclass(Sys.time())))){
