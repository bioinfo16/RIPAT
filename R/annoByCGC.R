#' @title Annotate integration sites by cancer-related genes.
#' 
#' @description
...
#' 
#' @export
annoByCGC = function(hits, ran_hits = NULL, mapTool = 'blast', organism = 'GRCh37', interval = 5000, range = c(-20000, 20000), outPath = getwd(), outFileName = paste0('RIPAT', round(unclass(Sys.time())))){
  ...
  dbFile = paste0('/', organism, '_cgc.rds')
  ...
  inside_tab = unique(cbind(only_res_que, only_res_sub)[,c(5,1:4,6,12,14,7:9,11,10)])
  names(inside_tab)[c(7:13)] = c('Target_gene', 'Target_gene_type', 'Target_chr', 'Target_start', 'Target_end', 'Target_strand', 'Target_gene_length')
  message('- OK!')
  ...
  dist_only = lapply(c(1:nrow(only_hits_tab)), function(a){
    x = as.numeric(only_hits_tab$start[a]); y = only_hits_tab$query[a]; z = only_hits_tab$seqnames[a]
  ...
  hist_only_g = hist(unlist(lapply(dist_only, function(x){x$dist/1000})), breaks = ranges/1000, plot = FALSE)
  message('- OK!')
  if(!is.null(ran_hits)){
    ...
    inside_ran_tab = unique(cbind(a,b)[,c(1,2,3,11,13,6:8,10)])
    names(inside_ran_tab) = c('q_chr', 'q_start', 'q_end', 'symbol', 'gene_type', 'chr', 'start', 'end', 'strand')
    ...
    all_dist_ran = unlist(lapply(dist_ran, function(x){x$dist}))
    hist_obj_ran = hist(all_dist_ran, plot = TRUE, breaks = ranges)
    message('- OK!')
  ...
  grDevices::dev.off()
  message('- OK!')
  result_list = list(inside_tab, g_dist, count_data, gr_genes, organism)
  names(result_list) = c('Gene_inside', 'Gene_distribution', 'Gene_plot_data', 'Gene_data', 'Target_ver')
  if(!is.null(ran_hits)){
    result_list = c(result_list, list(inside_ran_tab, dist_ran))
    names(result_list)[6:7] = c('Gene_inside_ran', 'Random_distribution')
  }
  result_list = c(result_list, list('Gene_hist_plot' = g_plot))
