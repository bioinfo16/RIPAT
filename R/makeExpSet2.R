...
#'          
#' @usage 
#' makeExpSet2(inDir, id,
#'             mapTool = 'blast', 
#'             vectorPos = 'front',
#'             outPath = getwd(), 
#'             outFileName = paste0('RIPAT', round(unclass(Sys.time()))))
#' 
#' @param inDir a string vector. Location of a directory that has a local alignment\cr
#'              result files. All alignment result files do not include\cr
#'              any header and comment.
#' @param id a character vector. the specific words that can appoint\cr
#'           alignment file names exclusively.
#' @param mapTool a character vector. Function serves two types of file\cr
#'                such as outputs from BLAST and BLAT. Default is 'blast'.\cr
#'                If you want to use BLAT result, use 'blat'.
#' @param vectorPos a character vector. Sets the position of vector on sequences.\cr
#'                  Default value is 'front'. If the vector is located at the behind of sequence,\cr
...
#' 
#' @return Return two types of outputs. Text file and R object.\cr
#'         Available hit data from input is written in text file and\cr
#'         generated as a list of GenomicRange(GR) format object.
#'         
#' @examples 
#' blast_obj = makeExpSet2(inDir = system.file("scripts", package = "RIPAT"),
#'                         id = 'BLASTn')
#'  
#' @export
makeExpSet2 = function(inDir, id, mapTool = 'blast', vectorPos = 'front', outPath = getwd(), outFileName = paste0('RIPAT', round(unclass(Sys.time())))){
